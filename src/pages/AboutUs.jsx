...
// admin can still override everything via Gym Settings (gymConfig wins).
const DEFAULTS = {
  gymName: 'Wild Bout Boxing Gym',
  foundingYear: 2021,
  tagline: 'Makati’s home for real boxing.',
  heroDescription: 'From first-timers to competitors, we train fighters of every level in the heart of Makati — with hands-on coaching, modern equipment, and a community that pushes you forward every round.',
  storyTitle: 'Built by fighters,\nfor fighters.',
  storyParagraphs: [
    'Wild Bout Boxing Gym opened in Makati with one goal: make authentic boxing training accessible to everyone — whether you are throwing your first jab or preparing for the amateur ring.',
    'Our coaches blend traditional boxing fundamentals with AI-assisted form tracking, so every member improves measurably with every session, not just by feel.',
    'More than a gym, we are a community. Members of every age and skill level train side by side, pushing each other to be sharper, stronger, and more confident inside and outside the ropes.',
  ],
  services: [
    'Beginner to advanced boxing classes',
    'One-on-one coaching & form correction',
    'AI pose-tracking punch analytics',
    'Strength & conditioning programs',
    'Open gym & heavy-bag access',
    'Amateur competition preparation',
  ],
  values: [
    { icon:'\u{1F94A}', title:'Discipline', desc:'Real progress comes from showing up and putting in the work, round after round.' },
    { icon:'\u{1F91D}', title:'Community', desc:'Every member belongs. We train hard and lift each other up.' },
    { icon:'\u{1F4C8}', title:'Progress', desc:'Data-driven training so you can see exactly how you improve over time.' },
    { icon:'\u{1F6E1}\u{FE0F}', title:'Safety', desc:'Proper technique and medical clearance first — we protect our fighters.' },
  ],
  address: 'Makati City, Metro Manila, Philippines',
  phone: '+63 917 000 0000',
  email: 'hello@wildboutboxing.ph',
  hoursLine: 'Mon–Fri 6AM–10PM · Sat–Sun 8AM–8PM',
  weeklyHours: [
...
export default function AboutUs() {
  ...
  const cfg = gymConfig || {}
  const gymName = cfg.gymName?.trim() || DEFAULTS.gymName
  const foundingYear = cfg.foundingYear ? Number(cfg.foundingYear) : DEFAULTS.foundingYear
  ...
  const tagline = cfg.tagline?.trim() || DEFAULTS.tagline
  const heroDescription = cfg.heroDescription?.trim() || cfg.aboutDescription?.trim() || DEFAULTS.heroDescription

  const storyTitle = cfg.storyTitle?.trim() || DEFAULTS.storyTitle
  let storyParagraphs = Array.isArray(cfg.storyParagraphs)
    ? cfg.storyParagraphs.filter(p => typeof p === 'string' && p.trim())
    : []
  if (storyParagraphs.length === 0) storyParagraphs = DEFAULTS.storyParagraphs

  let services = Array.isArray(cfg.services)
    ? cfg.services.filter(s => typeof s === 'string' && s.trim())
    : []
  if (services.length === 0) services = DEFAULTS.services

  let values = Array.isArray(cfg.values)
    ? cfg.values.filter(v => v && (v.title || v.desc))
    : []
  if (values.length === 0) values = DEFAULTS.values

  // Location / hours / pricing — defaults fill in when gymConfig lacks them
  const address = cfg.address?.trim() || DEFAULTS.address
  const weeklyHours = Array.isArray(cfg.weeklyHours) && cfg.weeklyHours.length > 0 ? cfg.weeklyHours : DEFAULTS.weeklyHours
  const mapQuery = cfg.mapQuery?.trim() || cfg.address?.trim() || DEFAULTS.mapQuery
  const plans = Array.isArray(cfg.plans) && cfg.plans.length > 0 ? cfg.plans : DEFAULTS.plans
  ...
  const contactFields = [
    { icon:'📍', label:'Location', val: cfg.address?.trim() || DEFAULTS.address },
    { icon:'📞', label:'Phone', val: cfg.phone?.trim() || DEFAULTS.phone },
    { icon:'📧', label:'Email', val: cfg.email?.trim() || DEFAULTS.email },
    { icon:'🕐', label:'Hours', val: cfg.hours?.trim() || DEFAULTS.hoursLine },
  ].filter(c => c.val)
      ...
      <div style={{position:'relative',zIndex:1,maxWidth:1200,margin:'0 auto',padding:'0 40px 60px',display:'flex',flexDirection:'column',gap:0,fontFamily:'Montserrat,sans-serif'}}>
          ...
          <div style={{...glass({borderRadius:24}),padding:0,display:'grid',gridTemplateColumns:'1.2fr 1fr',gap:0,overflow:'hidden'}}>
            ...
            <div style={{position:'relative',minHeight:320,background:'#0e0a0a'}}>
              <iframe
                ...
                style={{position:'absolute',inset:0,width:'100%',height:'100%',border:0,filter:'grayscale(0.3) contrast(1.1) brightness(0.85)'}}
                loading="lazy"
        ...
        <div style={{marginBottom:24}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(240px,1fr))',gap:18,alignItems:'stretch'}}>
            {plans.map((plan,i)=>{
              const isFree = String(plan.price).replace(/[^0-9]/g,'') === '0'
              const color = plan.highlight ? '#e84a2f' : '#f5c842'
              return (
