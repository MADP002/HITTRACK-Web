...
import { collection, getDocs, doc, getDoc, addDoc, deleteDoc, setDoc, updateDoc, serverTimestamp, query, where, orderBy, onSnapshot, writeBatch, arrayUnion, arrayRemove } from 'firebase/firestore'
...
// ── Completed-session helpers (Item 2) ────────────────
// Feedback now targets actual completed trainingSessions, not projected
// schedule days. These format a session into coach-readable labels.
function prettyTrainingName(id){
...
function sessionLabel(session){
  ...
  const name = prettyTrainingName(session.trainingId)
  const lvl  = session.level ? ` · ${capWord(session.level)}` : ''
  return `${name}${lvl}`
}
...
// ── MEMBER PANEL ──────────────────────────────────────
function MemberPanel({ selMember, setSelMember, setMsgTarget, setMsgThread, sessions, selSession, setSelSession, fbText, setFbText, fbRating, setFbRating, postFeedback, feedbackMap, editingFb, setEditingFb, saveEditFeedback, deleteFeedback, setConfirm, setLevelTarget }) {
  const lc = { Beginner:'#fb923c', Intermediate:'#f5c842', Advanced:'#22c55e' }[getMemberLevel(selMember)] || '#f5c842'
  const lvIcon = { Beginner:'🥊', Intermediate:'⚡', Advanced:'🔥' }[getMemberLevel(selMember)] || '🥊'
  const fbList = feedbackMap[selMember.uid] || []
  // Item 2 — feedback targets actual completed sessions, never future days.
  const selSessionObj = selSession ? (sessions || []).find(s => s.id === selSession) : null
  const selLabel = selSessionObj ? sessionLabel(selSessionObj) : null
  const latestSession = (sessions && sessions.length > 0) ? sessions[0] : null
  const memberRankPts = levelScore(selMember)
  ...
  return (
    <div style={{display:'flex', flexDirection:'column', gap:14}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
              <span style={{fontSize:9,fontWeight:700,padding:'3px 9px',borderRadius:50,textTransform:'uppercase',letterSpacing:'0.08em',background:`${lc}22`,color:lc,border:`1px solid ${lc}44`}}>
                {lvIcon} {getMemberLevel(selMember)}
              </span>
      ...
      {/* ═════════════════════════════════════════════ */}
      {/* MEDICAL CLEARANCE — Issue #6                   */}
      {/* Clearance-before-performance: confirm the     */}
      {/* member is safe to train before reviewing      */}
      {/* their metrics or sending feedback.            */}
      {/* ═════════════════════════════════════════════ */}
      <MedicalCertCard member={selMember} />

      {/* ═════════════════════════════════════════════ */}
      {/* PUNCH ANALYTICS — shared component, AI flagship */}
      {/* ═════════════════════════════════════════════ */}
      <PunchAnalyticsCard uid={selMember.uid} compact />
      ...
      <div style={{position:'relative',overflow:'hidden',borderRadius:20,background:'linear-gradient(135deg,rgba(20,15,15,0.95),rgba(10,7,7,0.98))',border:'1px solid rgba(245,200,66,0.12)',boxShadow:'0 20px 50px rgba(0,0,0,0.5)'}}>
        ...
        <div style={{padding:'18px 24px 24px',display:'flex',flexDirection:'column',gap:14}}>
          ...
          {/* Latest completed session — quick context (Item 2) */}
          {latestSession ? (
            <div style={{background:'linear-gradient(135deg,rgba(34,197,94,0.08),rgba(34,197,94,0.02))',border:'1px solid rgba(34,197,94,0.25)',borderRadius:12,padding:'12px 14px'}}>
              <div style={{fontSize:9,fontWeight:700,color:'#22c55e',letterSpacing:'0.12em',marginBottom:6,textTransform:'uppercase',display:'flex',alignItems:'center',gap:6}}>
                <span style={{display:'inline-block',width:14,height:2,background:'#22c55e'}}/>
                Latest completed session
              </div>
              <div style={{fontSize:13,fontWeight:700,color:'#f0ece8'}}>{sessionLabel(latestSession)}</div>
              <div style={{fontSize:10,color:'#888',marginTop:4}}>
                {fmtSessionDate(latestSession.completedAt)} &middot; {latestSession.properReps||0} reps &middot; {Math.round((latestSession.duration||0)/60)} min
              </div>
            </div>
          ) : (
            <div style={{background:'rgba(255,255,255,0.02)', borderRadius:10, padding:'10px 14px', fontSize:11, color:'#666', textAlign:'center', border:'1px dashed rgba(255,255,255,0.06)'}}>
              No completed training sessions yet for this member.
            </div>
          )}

          {/* Session selector — completed sessions only (Item 2) */}
          <div>
            <div style={{fontSize:10,fontWeight:700,color:'#666',letterSpacing:'0.18em',marginBottom:10,textTransform:'uppercase',display:'flex',alignItems:'center',gap:10}}>
              ...
              Pick a completed session to comment on
            </div>

            <div style={{display:'flex',flexDirection:'column',gap:8,maxHeight:340,overflowY:'auto',paddingRight:4}}>

              {/* General feedback card */}
              <div onClick={() => setSelSession(null)}
                style={{position:'relative',background:selSession===null?'linear-gradient(135deg,rgba(245,200,66,0.12),rgba(232,74,47,0.06))':'linear-gradient(135deg,rgba(40,30,28,0.5),rgba(20,15,14,0.7))',border:`1px solid ${selSession===null?'rgba(245,200,66,0.45)':'rgba(255,255,255,0.06)'}`,borderRadius:14,padding:'14px 16px',display:'flex',alignItems:'center',gap:14,cursor:'pointer',transform:selSession===null?'translateX(4px)':'translateX(0)',boxShadow:selSession===null?'0 8px 24px rgba(245,200,66,0.15)':'none',transition:'all 0.3s cubic-bezier(0.34,1.56,0.64,1)',minHeight:74}}
                onMouseEnter={e=>{if(selSession!==null){e.currentTarget.style.transform='translateX(4px)';e.currentTarget.style.borderColor='rgba(245,200,66,0.3)'}}}
                onMouseLeave={e=>{if(selSession!==null){e.currentTarget.style.transform='translateX(0)';e.currentTarget.style.borderColor='rgba(255,255,255,0.06)'}}}>
                <div style={{position:'absolute',left:0,top:0,bottom:0,width:selSession===null?5:0,background:'linear-gradient(180deg,#f5c842,transparent)',transition:'width 0.3s ease',borderRadius:'14px 0 0 14px'}}/>
                ...
                <div style={{flex:1,minWidth:0,display:'flex',flexDirection:'column',justifyContent:'center',gap:4}}>
                  <div style={{display:'flex',alignItems:'center',gap:6,flexWrap:'wrap'}}>
                    <span style={{fontSize:13,fontWeight:700,color:selSession===null?'#f5c842':'#f0ece8'}}>General Feedback</span>
                    {selSession===null && <span style={{fontSize:8,fontWeight:800,letterSpacing:'0.1em',padding:'2px 7px',borderRadius:50,background:'rgba(245,200,66,0.2)',color:'#f5c842',flexShrink:0}}>SELECTED</span>}
                  </div>
                  <div style={{fontSize:10,color:'#777'}}>Not tied to a specific session</div>
                </div>
                <div style={{width:24,height:24,borderRadius:'50%',border:`1.5px solid ${selSession===null?'#f5c842':'rgba(255,255,255,0.15)'}`,display:'flex',alignItems:'center',justifyContent:'center',background:selSession===null?'linear-gradient(135deg,#f5c842,#e08820)':'transparent',color:'#000',fontWeight:700,fontSize:12,transform:selSession===null?'scale(1.1)':'scale(1)',transition:'all 0.3s ease',flexShrink:0}}>
                  {selSession===null?'✓':''}
                </div>
              </div>
              ...
              {(!sessions || sessions.length === 0) && (
                <div style={{background:'rgba(255,255,255,0.02)',borderRadius:12,padding:'16px',fontSize:11,color:'#666',textAlign:'center',border:'1px dashed rgba(255,255,255,0.06)'}}>
                  No completed sessions to comment on yet. Leave General Feedback above, or wait for the member to finish a session on the mobile app.
                </div>
              )}

              {/* Each completed session */}
              {(sessions || []).map(s => {
                const sel = selSession === s.id
                return (
                  <div key={s.id} onClick={() => setSelSession(s.id)}
                    style={{position:'relative',background:sel?'linear-gradient(135deg,rgba(66,165,245,0.10),rgba(66,165,245,0.03))':'linear-gradient(135deg,rgba(34,197,94,0.05),rgba(20,15,14,0.7))',border:`1px solid ${sel?'rgba(66,165,245,0.45)':'rgba(34,197,94,0.18)'}`,borderRadius:14,padding:'14px 16px',display:'flex',alignItems:'center',gap:14,cursor:'pointer',transform:sel?'translateX(4px)':'translateX(0)',boxShadow:sel?'0 8px 24px rgba(66,165,245,0.15)':'none',transition:'all 0.3s cubic-bezier(0.34,1.56,0.64,1)',minHeight:74}}
                    onMouseEnter={e=>{if(!sel){e.currentTarget.style.transform='translateX(4px)';e.currentTarget.style.borderColor='rgba(34,197,94,0.4)'}}}
                    onMouseLeave={e=>{if(!sel){e.currentTarget.style.transform='translateX(0)';e.currentTarget.style.borderColor='rgba(34,197,94,0.18)'}}}>
                    <div style={{position:'absolute',left:0,top:0,bottom:0,width:sel?5:0,background:`linear-gradient(180deg,${sel?'#42a5f5':'#22c55e'},transparent)`,transition:'width 0.3s ease',borderRadius:'14px 0 0 14px'}}/>
                    <div style={{width:46,height:46,borderRadius:12,background:'linear-gradient(135deg,rgba(34,197,94,0.25),rgba(34,197,94,0.05))',border:'1px solid rgba(34,197,94,0.4)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,flexShrink:0}}>&#129354;</div>
                    <div style={{flex:1,minWidth:0,display:'flex',flexDirection:'column',justifyContent:'center',gap:4}}>
                      <div style={{display:'flex',alignItems:'center',gap:6,flexWrap:'wrap'}}>
                        <span style={{fontSize:13,fontWeight:700,color:sel?'#42a5f5':'#22c55e',lineHeight:1.3}}>{prettyTrainingName(s.trainingId)}</span>
                        {s.level && <span style={{fontSize:8,fontWeight:800,letterSpacing:'0.08em',padding:'2px 7px',borderRadius:50,background:'rgba(34,197,94,0.18)',color:'#22c55e',flexShrink:0,textTransform:'uppercase'}}>{capWord(s.level)}</span>}
                        {sel && <span style={{fontSize:8,fontWeight:800,letterSpacing:'0.1em',padding:'2px 7px',borderRadius:50,background:'rgba(66,165,245,0.18)',color:'#42a5f5',flexShrink:0}}>SELECTED</span>}
                      </div>
                      <div style={{fontSize:10,color:'#777',lineHeight:1.4}}>
                        {fmtSessionDate(s.completedAt)} &middot; {s.properReps||0} reps &middot; {Math.round((s.duration||0)/60)} min
                      </div>
                    </div>
          ...
          {selSessionObj && (
            <div style={{background:'linear-gradient(135deg,rgba(66,165,245,0.08),rgba(66,165,245,0.02))',border:'1px solid rgba(66,165,245,0.25)',borderRadius:12,padding:'12px 14px'}}>
              ...
              <div style={{fontSize:12,fontWeight:700,color:'#f0ece8',marginBottom:4}}>{sessionLabel(selSessionObj)}</div>
              <div style={{fontSize:10,color:'#888'}}>
                Completed {fmtSessionDate(selSessionObj.completedAt)} &middot; {selSessionObj.properReps||0} proper reps &middot; {Math.round((selSessionObj.duration||0)/60)} min
              </div>
            </div>
          )}
          ...
          <div>
            ...
            <div style={{position:'relative'}}>
              <textarea
                ...
                rows={4}
                maxLength={500}
                placeholder={selLabel ? `Tell ${selMember.name?.split(' ')[0]||'them'} on their ${selLabel} session...` : `Encourage ${selMember.name?.split(' ')[0]||'them'}, share a coaching tip, or push their next goal...`}
          ...
          <button onClick={postFeedback} disabled={!fbText.trim() || fbRating === 0}
            ...
            🥊 Send Feedback {selLabel ? '— ' + (selLabel||'') : '(General)'} →
          </button>
        </div>

        {/* Past feedback — always rendered so coaches know the section exists, even when empty */}
        <div style={{borderTop:'1px solid rgba(255,255,255,0.06)',padding:'18px 24px',display:'flex',flexDirection:'column',gap:10,background:'rgba(0,0,0,0.2)'}}>
          ...
          {fbList.length === 0 && (
            <div style={{padding:'18px 14px',textAlign:'center',background:'rgba(255,255,255,0.02)',border:'1px dashed rgba(255,255,255,0.08)',borderRadius:12}}>
              <div style={{fontSize:24,marginBottom:6,opacity:0.5}}>💬</div>
              <div style={{fontSize:12,fontWeight:700,color:'#888',marginBottom:3}}>No feedback sent to this member yet</div>
              <div style={{fontSize:10,color:'#666',lineHeight:1.5}}>Use the composer above to send your first piece of feedback.</div>
            </div>
          )}
          {fbList.length > 0 && fbList.map((fb, i) => (
...
export default function CoachDashboard(){
  ...
  // Training Reports tab — Issue #5. Members submit reports to a coach via
  // mobile training-complete; coach reads them here. Rules already allow
  // coach read + update where coachUid == coach.uid (firestore.rules:222-233).
  ...
  // Real, manually-posted gym announcements only. Item 3 — also exclude
  // member-directed notifications (renewal reminders, level-up popups, etc.)
  // so they never leak into the coach's announcement feed.
  const realAnnouncements = notifs.filter(n => {
    if (n.type && SYSTEM_EVENT_TYPES.includes(n.type)) return false
    if (n.audience === 'member') return false                                  // member-only audience
    if (n.targetUserId && n.targetUserId !== auth.currentUser?.uid) return false // directed at someone else
    return true
  })
  ...
  // Hidden by THIS coach — Issue #9, shown in the unhide panel.
  ...
  // Unhide panel — Issue #9. Lets the coach restore announcements they've previously
  // dismissed from their view. Hidden state lives in users/{coachUid}.dismissedNotifs;
  // restoring is just arrayRemove, real-time onSnapshot picks the change up.
  const [showHiddenPanel, setShowHiddenPanel] = useState(false)

  async function restoreAnnouncement(notifId) {
    const me = auth.currentUser
    if (!me) return
    try {
      await updateDoc(doc(db, 'users', me.uid), {
        dismissedNotifs: arrayRemove(notifId),
      })
  ...
  async function restoreAllAnnouncements() {
    const me = auth.currentUser
    if (!me || dismissedNotifs.length === 0) return
    try {
      await updateDoc(doc(db, 'users', me.uid), {
        dismissedNotifs: arrayRemove(...dismissedNotifs),
      })
      showToast(`↺ Restored ${dismissedNotifs.length} announcement${dismissedNotifs.length===1?'':'s'}`)
  ...
  useEffect(()=>{
    ...
    const q=query(collection(db,'messages'),where('participants','array-contains',coachUid))
  ...
  // Item 2 — feedback now targets actual completed sessions.
  const [memberSessions,setMemberSessions]=useState({})   // uid -> array of trainingSessions docs (newest first)
  const [selSession,setSelSession]=useState(null)         // selected session id, or null = general feedback
  ...
  // Item 2 — load this member's actually-completed training sessions.
  // Coaches can read any member's trainingSessions (firestore.rules:203-214).
  // Filter by uid only, sort client-side by completedAt (no composite index).
  async function loadMemberSessions(uid){
    try{
      const q=query(collection(db,'trainingSessions'),where('uid','==',uid))
      const snap=await getDocs(q)
      const sessions=snap.docs.map(d=>({id:d.id,...d.data()}))
        .sort((a,b)=>(b.completedAt?.seconds||0)-(a.completedAt?.seconds||0))
      setMemberSessions(prev=>({...prev,[uid]:sessions}))
  ...
  // Live subscription: training reports sent to this coach.
  // Filter by coachUid (rules require it for read); sort client-side by
  // submittedAt to avoid a composite index.
  useEffect(()=>{
    const me=auth.currentUser
    if(!me){setReportsLoading(false);return}
    setReportsLoading(true)
    const q=query(collection(db,'trainingRecordings'),where('coachUid','==',me.uid))
    const unsub=onSnapshot(q,snap=>{
      ...
      console.warn('Reports listener:',err.message)
      setReportsLoading(false)
    })
    return()=>unsub()
  },[])

  // Toggle expand. First open marks the report viewed (rules already allow
  // the assigned coach to update). No-op if already viewed.
  async function openReport(report){
    if(expandedReport===report.id){setExpandedReport(null);return}
    setExpandedReport(report.id)
    if(!report.viewed){
      try{await updateDoc(doc(db,'trainingRecordings',report.id),{viewed:true})}
  ...
  useEffect(()=>{
    if(selMember){
      ...
      loadMemberSessions(selMember.uid)
      setSelSession(null)
      setEditingFb(null)
      setFbText('')
      setFbRating(0)
  ...
  async function postFeedback(){
    ...
    // Item 2 — feedback targets a real completed session (or General).
    const sessions = memberSessions[selMember.uid] || []
    const session  = selSession ? sessions.find(s => s.id === selSession) : null
    ...
    try{
      await addDoc(collection(db,'feedback'),{
        ...
        // New: tie to an actual completed session
        sessionId:        session ? session.id : null,
        sessionTrainingId:session ? (session.trainingId || null) : null,
        sessionLevel:     session ? (session.level || null) : null,
        sessionCompletedAt: session ? (session.completedAt || null) : null,
        // Back-compat: feedback list still reads workoutDayLabel
        workoutDayIndex:  null,
        workoutDayLabel:  label,
        createdAt:serverTimestamp(),
      })
      setFbText('');setFbRating(0);setSelSession(null)
      loadFeedback(selMember.uid)
  ...
  async function changeMemberLevel(member, newLevel){
    ...
    try {
      const me = auth.currentUser
      // 1. Update users doc. firestore.rules only lets a coach write
      //    ['experience','status'] here — the canonical resolver returns
      //    experience for Intermediate/Advanced, so promotes take effect
      //    without needing to also write trainingLevel.
      await updateDoc(doc(db,'users',member.uid), { experience: newLevel })
      // 2. Update stats doc (rules limit coach to ['experience'] here too).
      try {
        ...
        try {
          if (bk.userId) {
            await addDoc(collection(db, 'notifications'), {
              ...
              message:      `Your booking for "${cls?.name || 'a class'}" on ${cls?.day || ''} at ${cls?.time || ''} has been cancelled by the coach. Sorry for the inconvenience!`,
              audience:     'member',
  ...
  // Coach-side renewal nudge — drops a notification into the member's feed.
  // De-duped per-session so a coach can't accidentally spam someone.
  async function sendMembershipReminder(member){
    if(!member?.uid)return
    if(remindedThisCycle[member.uid]){showToast('Already reminded this session','error');return}
    try{
      const days=daysRemaining(member.membership)
      const dayLabel=days===null?'soon':days<0?`${Math.abs(days)} day${Math.abs(days)===1?'':'s'} ago`:days===0?'today':`in ${days} day${days===1?'':'s'}`
      await addDoc(collection(db,'notifications'),{
  ...
  const progressFiltered=[...members].filter(m=>{
    if(progLevel!=='All Levels'&&getMemberLevel(m)!==progLevel)return false
    ...
    <div style={{minHeight:'100vh',background:'#0c0a0a',fontFamily:'Montserrat,sans-serif',position:'relative'}}>
      ...
      <div style={{position:'relative',zIndex:1,maxWidth:1400,margin:'0 auto',padding:'20px 32px 60px',display:'flex',flexDirection:'column',gap:16}}>
                        ...
                        <div key={m.uid} onClick={()=>setSelMember(isSelected?null:m)}
                          ...
                          <div style={{flex:1,minWidth:0}}>
                            ...
                            <div style={{display:'flex',gap:5,alignItems:'center',flexWrap:'wrap'}}>
                              <span style={{fontSize:8,fontWeight:700,padding:'1px 6px',borderRadius:50,background:`${lc}15`,color:lc,letterSpacing:'0.06em',textTransform:'uppercase'}}>{getMemberLevel(m)}</span>
            ...
            {selMember&&(
              <div style={{overflowY:'auto',maxHeight:'90vh',paddingRight:4}}>
                <MemberPanel
                  ...
                  sessions={memberSessions[selMember.uid] || []}
                  selSession={selSession}
                  setSelSession={setSelSession}
                  fbText={fbText}
                  setFbText={setFbText}
                  fbRating={fbRating}
                    ...
                    return(
                      <div key={m.uid}
                        ...
                        <div style={{width:160,flexShrink:0,minWidth:0}}>
                          ...
                          <span style={{fontSize:8,fontWeight:700,padding:'2px 7px',borderRadius:50,background:`${lc}15`,color:lc,letterSpacing:'0.06em',textTransform:'uppercase'}}>{getMemberLevel(m)}</span>
                        </div>
                        {/* Progress bar */}
        ...
        {tab==='reports'&&(
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
            {/* Header strip */}
            <div style={{position:'relative',overflow:'hidden',borderRadius:14,background:'linear-gradient(135deg,#1a1413 0%,#0e0a0a 100%)',border:'1px solid rgba(245,200,66,0.12)'}}>
              <div style={{position:'absolute',left:0,top:0,bottom:0,width:4,background:'linear-gradient(180deg,#f5c842,#e84a2f)'}}/>
              <div style={{padding:'14px 18px 14px 22px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,flexWrap:'wrap'}}>
                <div>
                  <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,letterSpacing:'0.06em',color:'#f0ece8',lineHeight:1}}>📋 TRAINING REPORTS</div>
                  <div style={{fontSize:11,color:'#888',marginTop:3}}>Reports submitted to you by your members</div>
                </div>
                <div style={{display:'flex',gap:8,alignItems:'center'}}>
                  <div style={{fontSize:9,fontWeight:800,padding:'4px 10px',borderRadius:50,background:'rgba(245,200,66,0.15)',color:'#f5c842',letterSpacing:'0.1em'}}>
                    {reports.length} TOTAL
                  </div>
                  {unviewedReports>0&&(
                    <div style={{fontSize:9,fontWeight:800,padding:'4px 10px',borderRadius:50,background:'rgba(232,74,47,0.15)',color:'#e84a2f',letterSpacing:'0.1em',display:'flex',alignItems:'center',gap:6}}>
                      <span style={{display:'inline-block',width:6,height:6,borderRadius:'50%',background:'#e84a2f',animation:'pulseDot 1.6s ease-in-out infinite'}}/>
                      {unviewedReports} NEW
                    </div>
                  )}
                </div>
            ...
            {reportsLoading ? (
              <div style={{padding:50,textAlign:'center',color:'#555',fontSize:12}}>Loading training reports...</div>
            ) : reports.length===0 ? (
              <div style={{padding:'60px 24px',textAlign:'center',background:'linear-gradient(135deg,#1a1413 0%,#0e0a0a 100%)',borderRadius:14,border:'1px dashed rgba(255,255,255,0.08)'}}>
                <div style={{fontSize:48,marginBottom:12,opacity:0.6}}>📋</div>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,color:'#f0ece8',letterSpacing:'0.04em',marginBottom:6}}>No training reports yet</div>
                <div style={{fontSize:12,color:'#666',lineHeight:1.6,maxWidth:340,margin:'0 auto'}}>
                  When one of your members completes a training session and sends you the report, it will appear here.
                </div>
              </div>
            ) : (
              <div style={{display:'flex',flexDirection:'column',gap:10}}>
                {reports.map(r=>{
                  ...
                  const submittedDate=r.submittedAt?.toDate?r.submittedAt.toDate():(r.submittedAt?.seconds?new Date(r.submittedAt.seconds*1000):null)
                  const lvl=r.level||'beginner'
                  const lvlCap=String(lvl).charAt(0).toUpperCase()+String(lvl).slice(1).toLowerCase()
                  const lvlColor=LEVEL_COLOR[lvlCap]||'#f5c842'
                  const isNew=!r.viewed
                  return (
                    <div key={r.id} style={{position:'relative',overflow:'hidden',borderRadius:14,background:'linear-gradient(135deg,#1a1413 0%,#0e0a0a 100%)',border:`1px solid ${isNew?'rgba(232,74,47,0.35)':'rgba(255,255,255,0.06)'}`,transition:'all 0.25s'}}>
                      <div style={{position:'absolute',left:0,top:0,bottom:0,width:4,background:isNew?'linear-gradient(180deg,#e84a2f,#c93820)':`linear-gradient(180deg,${lvlColor}aa,${lvlColor}33)`}}/>
                      {/* Row header — always visible, click to expand */}
                      <div onClick={()=>openReport(r)}
                        style={{padding:'14px 18px 14px 22px',display:'flex',alignItems:'center',gap:14,cursor:'pointer',userSelect:'none'}}
                        onMouseEnter={e=>{e.currentTarget.style.background='rgba(255,255,255,0.02)'}}
                        onMouseLeave={e=>{e.currentTarget.style.background='transparent'}}>
                        {/* Avatar */}
                        <div style={{width:42,height:42,borderRadius:'50%',background:`linear-gradient(135deg,${lvlColor},${lvlColor}aa)`,color:'#000',border:`2px solid ${lvlColor}66`,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'Bebas Neue',sans-serif",fontSize:18,flexShrink:0}}>
                          {(r.memberName||'?')[0].toUpperCase()}
                        </div>
                        {/* Name + meta */}
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap',marginBottom:3}}>
                            <span style={{fontSize:13,fontWeight:700,color:'#f0ece8'}}>{r.memberName||'Member'}</span>
                            {isNew&&(
                              <span style={{fontSize:8,fontWeight:800,padding:'2px 7px',borderRadius:50,background:'rgba(232,74,47,0.18)',color:'#e84a2f',border:'1px solid rgba(232,74,47,0.4)',letterSpacing:'0.1em'}}>NEW</span>
                            )}
                            <span style={{fontSize:8,fontWeight:800,padding:'2px 7px',borderRadius:50,background:`${lvlColor}1a`,color:lvlColor,border:`1px solid ${lvlColor}44`,letterSpacing:'0.08em',textTransform:'uppercase'}}>
                              {lvlCap}
                            </span>
                          </div>
                          <div style={{fontSize:11,color:'#888',display:'flex',gap:10,flexWrap:'wrap'}}>
                            <span>🥊 {r.trainingName||r.trainingId||'Training'}</span>
                            <span>· {r.properReps||0} reps</span>
                            <span>· {Math.round((r.duration||0)/60)} min</span>
                          </div>
                        </div>
                        ...
                        <div style={{fontSize:16,color:'#555',transition:'transform 0.2s',transform:expanded?'rotate(180deg)':'rotate(0)'}}>▾</div>
                      </div>

                      {/* Expanded panel — performance metrics */}
                      {expanded&&(
                        <div style={{padding:'4px 18px 18px 22px',borderTop:'1px solid rgba(255,255,255,0.04)'}}>
                          <div style={{fontSize:9,fontWeight:800,color:'#888',letterSpacing:'0.14em',textTransform:'uppercase',marginTop:14,marginBottom:10}}>
                            Performance Breakdown
                          </div>
                          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit, minmax(120px, 1fr))',gap:10}}>
                            {[
                              {icon:'✨',label:'Form Quality', val:r.avgQualityPct!=null?`${r.avgQualityPct}%`:'—',color:'#f5c842'},
                              {icon:'⚡',label:'Pace',         val:r.paceRepsPerMin!=null?`${r.paceRepsPerMin}/min`:'—',color:'#42a5f5'},
                              {icon:'📊',label:'Consistency',  val:r.consistencyPct!=null?`${r.consistencyPct}%`:'—',color:'#4ade80'},
                              {icon:'🔥',label:'Best Streak',  val:r.bestStreak!=null?String(r.bestStreak):'—',color:'#e84a2f'},
                            ].map((m,i)=>(
                              <div key={i} style={{background:`${m.color}0c`,border:`1px solid ${m.color}22`,borderRadius:12,padding:'12px 14px'}}>
                                <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:6}}>
                                  <span style={{fontSize:16}}>{m.icon}</span>
                                  <span style={{fontSize:9,color:'#888',fontWeight:700,letterSpacing:'0.08em',textTransform:'uppercase'}}>{m.label}</span>
                                </div>
                                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:m.color,letterSpacing:'0.02em'}}>{m.val}</div>
                              </div>
                            ))}
                          </div>
                          <div style={{marginTop:14,padding:'10px 14px',background:'rgba(66,165,245,0.06)',border:'1px solid rgba(66,165,245,0.18)',borderRadius:10,fontSize:11,color:'#7ba8d4',lineHeight:1.6}}>
                            <strong style={{color:'#42a5f5'}}>Training Report</strong> · submitted {submittedDate?submittedDate.toLocaleString():'—'}.
                            {' '}Marked viewed automatically. To reply, open the member from the Members tab and send feedback.
                          </div>
                        </div>
                      )}
                    </div>
        ...
        {tab==='notifications'&&(
          <div style={{display:'flex',flexDirection:'column',gap:14}}>
              ...
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                ...
                {dismissedNotifs.length > 0 && (
                  <button onClick={()=>setShowHiddenPanel(v=>!v)}
                    title="Click to view and restore announcements you've previously hidden"
                    style={{fontSize:9,fontWeight:800,padding:'4px 10px',borderRadius:50,
                      background: showHiddenPanel ? 'rgba(245,200,66,0.18)' : 'rgba(120,113,108,0.18)',
                      color: showHiddenPanel ? '#f5c842' : '#bbb',
                      border: `1px solid ${showHiddenPanel ? 'rgba(245,200,66,0.4)' : 'rgba(255,255,255,0.1)'}`,
                      letterSpacing:'0.08em', cursor:'pointer', transition:'all 0.2s',
                      display:'inline-flex', alignItems:'center', gap:5}}
                    onMouseEnter={e=>{if(!showHiddenPanel){e.currentTarget.style.background='rgba(120,113,108,0.28)';e.currentTarget.style.color='#fff'}}}
                    onMouseLeave={e=>{if(!showHiddenPanel){e.currentTarget.style.background='rgba(120,113,108,0.18)';e.currentTarget.style.color='#bbb'}}}>
                    👁‍🗨 {dismissedNotifs.length} HIDDEN <span style={{fontSize:9,opacity:0.7}}>{showHiddenPanel?'▴':'▾'}</span>
                  </button>
                )}
              </div>
              <div style={{display:'flex',alignItems:'center',gap:8}}>
            ...
            {/* HIDDEN ANNOUNCEMENTS PANEL — Issue #9. Toggled by the HIDDEN badge above. */}
            {showHiddenPanel && (
              <div style={{position:'relative',overflow:'hidden',borderRadius:14,background:'linear-gradient(135deg,#1a1413 0%,#0e0a0a 100%)',border:'1px solid rgba(120,113,108,0.3)'}}>
                <div style={{position:'absolute',left:0,top:0,bottom:0,width:4,background:'linear-gradient(180deg,#888,#555)'}}/>
                <div style={{padding:'14px 18px 14px 22px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,flexWrap:'wrap',borderBottom:hiddenAnnouncements.length>0?'1px solid rgba(255,255,255,0.05)':'none'}}>
                  <div>
                    <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:16,letterSpacing:'0.06em',color:'#f0ece8',lineHeight:1}}>👁‍🗨 HIDDEN ANNOUNCEMENTS</div>
                    <div style={{fontSize:10,color:'#888',marginTop:3,lineHeight:1.5}}>Only hidden from <em>your</em> view. Restoring brings them back to your Announcements list.</div>
                  </div>
                  {hiddenAnnouncements.length>0 && (
                    <button onClick={restoreAllAnnouncements}
                      style={{background:'rgba(74,222,128,0.12)',color:'#4ade80',border:'1px solid rgba(74,222,128,0.4)',borderRadius:50,padding:'8px 14px',fontSize:10,fontWeight:800,letterSpacing:'0.08em',cursor:'pointer',whiteSpace:'nowrap',transition:'all 0.2s'}}
                      onMouseEnter={e=>{e.currentTarget.style.background='rgba(74,222,128,0.22)';e.currentTarget.style.transform='translateY(-1px)'}}
                      onMouseLeave={e=>{e.currentTarget.style.background='rgba(74,222,128,0.12)';e.currentTarget.style.transform='translateY(0)'}}>
                      ↺ RESTORE ALL
                    </button>
                  )}
                </div>

                <div style={{padding:hiddenAnnouncements.length>0?'10px 14px 14px':0,display:'flex',flexDirection:'column',gap:8}}>
                  {hiddenAnnouncements.map(n=>{
                    const dt = n.createdAt?.toDate ? n.createdAt.toDate() : (n.createdAt?.seconds ? new Date(n.createdAt.seconds*1000) : null)
                    return (
                      <div key={n.id} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 14px',background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.05)',borderRadius:10,opacity:0.85}}>
                        <div style={{width:30,height:30,borderRadius:8,background:'rgba(245,200,66,0.12)',border:'1px solid rgba(245,200,66,0.3)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,flexShrink:0}}>📢</div>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:12,fontWeight:700,color:'#bbb',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{n.title || n.body?.slice(0,60) || 'Announcement'}</div>
                          <div style={{fontSize:10,color:'#666',marginTop:2}}>
                            {n.fromName ? `By ${n.fromName}` : ''} {dt ? `· ${dt.toLocaleDateString('en-US',{month:'short',day:'numeric'})}` : ''}
                          </div>
                        </div>
                        <button onClick={()=>restoreAnnouncement(n.id)}
                          title="Restore to your Announcements list"
                          style={{background:'rgba(74,222,128,0.1)',color:'#4ade80',border:'1px solid rgba(74,222,128,0.35)',borderRadius:50,padding:'6px 12px',fontSize:10,fontWeight:800,letterSpacing:'0.06em',cursor:'pointer',whiteSpace:'nowrap',transition:'all 0.2s'}}
                  ...
                  return(
                    <div key={n.id}
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        ...
                        <div style={{fontSize:9,color:'#555',fontWeight:600,letterSpacing:'0.05em',display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}}>
                          <span>By <strong style={{color:'#777'}}>{n.from}</strong></span>
                          {(() => {
                            const d = n.createdAt?.toDate ? n.createdAt.toDate() : (n.createdAt?.seconds ? new Date(n.createdAt.seconds*1000) : null)
                            if (!d) return null
                            return (
                              <>
                                <span style={{color:'#444'}}>·</span>
                                <span title={d.toLocaleString()} style={{color:'#666'}}>
                      ...
                      <button onClick={()=>dismissAnnouncement(n.id)} title="Hide from your view — others still see it. Restore anytime from the HIDDEN panel."
                        style={{alignSelf:'flex-start',flexShrink:0,width:32,height:32,background:'rgba(120,113,108,0.12)',color:'#a8a29e',border:'1px solid rgba(120,113,108,0.3)',borderRadius:9,display:'flex',alignItems:'center',justifyContent:'center',fontSize:14,cursor:'pointer',transition:'all 0.25s',padding:0}}
                        onMouseEnter={e=>{e.currentTarget.style.background='rgba(245,200,66,0.2)';e.currentTarget.style.color='#f5c842';e.currentTarget.style.borderColor='rgba(245,200,66,0.4)';e.currentTarget.style.transform='scale(1.1)';e.currentTarget.style.boxShadow='0 4px 12px rgba(245,200,66,0.25)'}}
        ...
        {/* MEMBERSHIPS — read-only roster of every member's plan status + reminder action */}
        {tab==='memberships' && (() => {
          const memberOnly = members.filter(m => (m.role||'member') === 'member' && m.status !== 'inactive')
          const active  = memberOnly.filter(x => computeMembershipState(x.membership) === STATUS.ACTIVE).length
          const trial   = memberOnly.filter(x => computeMembershipState(x.membership) === STATUS.TRIAL).length
          const expired = memberOnly.filter(x => computeMembershipState(x.membership) === STATUS.EXPIRED).length
          const paused  = memberOnly.filter(x => computeMembershipState(x.membership) === STATUS.PAUSED).length
          const expSoon = memberOnly.filter(x => isExpiringSoon(x.membership)).length

          const statusChips = [
            {id:'all',     label:'ALL',     count:memberOnly.length, color:'#f0ece8'},
            {id:'active',  label:'ACTIVE',  count:active,            color:getStatusColor(STATUS.ACTIVE)},
            {id:'trial',   label:'TRIAL',   count:trial,             color:getStatusColor(STATUS.TRIAL)},
            {id:'expired', label:'EXPIRED', count:expired,           color:getStatusColor(STATUS.EXPIRED)},
            {id:'paused',  label:'PAUSED',  count:paused,            color:getStatusColor(STATUS.PAUSED)},
            {id:'soon',    label:'EXPIRING SOON', count:expSoon,     color:'#fb923c'},
          ]

          const filtered = memberOnly
            .filter(m => {
              if (memSearch.trim() && !(m.name||'').toLowerCase().includes(memSearch.toLowerCase()) && !(m.email||'').toLowerCase().includes(memSearch.toLowerCase())) return false
              const st = computeMembershipState(m.membership)
              if (memStatusFilter === 'all')     return true
              if (memStatusFilter === 'active')  return st === STATUS.ACTIVE
              if (memStatusFilter === 'trial')   return st === STATUS.TRIAL
              if (memStatusFilter === 'expired') return st === STATUS.EXPIRED
              if (memStatusFilter === 'paused')  return st === STATUS.PAUSED
              if (memStatusFilter === 'soon')    return isExpiringSoon(m.membership)
              return true
            })
            .sort((a,b) => {
          ...
          return (
            <div style={{display:'flex',flexDirection:'column',gap:14}}>
              <div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
                <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,letterSpacing:'0.06em',color:'#f0ece8'}}>💳 MEMBERSHIPS</div>
                <span style={{fontSize:9,fontWeight:800,padding:'3px 9px',borderRadius:50,background:'rgba(66,165,245,0.15)',color:'#42a5f5',letterSpacing:'0.08em'}}>READ-ONLY</span>
                <span style={{fontSize:10,color:'#666',marginLeft:'auto'}}>Only admin can record payments or pause plans</span>
              </div>

              {/* Status chips */}
              <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                {statusChips.map(s => {
                  const active = memStatusFilter === s.id
                  return (
                    <button key={s.id} onClick={()=>setMemStatusFilter(s.id)}
                      style={{display:'flex',alignItems:'center',gap:8,background:active?`${s.color}18`:'rgba(255,255,255,0.03)',border:active?`1.5px solid ${s.color}55`:'1px solid rgba(255,255,255,0.06)',borderRadius:12,padding:'9px 14px',fontSize:10,fontWeight:700,color:active?s.color:'#777',cursor:'pointer',transition:'all 0.2s',letterSpacing:'0.07em'}}>
                      {s.label}
                      <span style={{fontSize:9,fontWeight:800,padding:'2px 7px',borderRadius:50,background:active?`${s.color}28`:'rgba(255,255,255,0.05)',color:active?s.color:'#888'}}>{s.count}</span>
                    </button>
                  )
                })}
              </div>
              ...
              {/* Search */}
              <div style={{position:'relative'}}>
                <span style={{position:'absolute',left:14,top:'50%',transform:'translateY(-50%)',fontSize:13,color:'#555'}}>🔍</span>
                <input placeholder="Search by name or email…" value={memSearch} onChange={e=>setMemSearch(e.target.value)}
                  style={{...inp,paddingLeft:38,borderRadius:12,background:'rgba(20,15,14,0.7)'}}
              ...
              {/* List */}
              {filtered.length === 0 ? (
                <div style={{background:'linear-gradient(135deg,#1a1413 0%,#0e0a0a 100%)',borderRadius:18,border:'1px dashed rgba(255,255,255,0.08)',padding:'50px 30px',textAlign:'center'}}>
                  <div style={{fontSize:48,marginBottom:10,opacity:0.4}}>💳</div>
                  <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:20,color:'#f0ece8',letterSpacing:'0.06em'}}>NO MEMBERS MATCH</div>
                  <div style={{fontSize:11,color:'#666',marginTop:6}}>Try a different filter or search term</div>
                </div>
              ) : (
                <div style={{display:'flex',flexDirection:'column',gap:8}}>
                  {filtered.map(m => {
                    ...
                    const alreadyReminded = !!remindedThisCycle[m.uid]
                    return (
                      <div key={m.uid}
                        style={{position:'relative',overflow:'hidden',background:'linear-gradient(135deg,#1a1413 0%,#0e0a0a 100%)',borderRadius:14,border:`1px solid ${sc}22`,padding:'14px 18px',display:'flex',gap:14,alignItems:'center',transition:'all 0.3s'}}
                        onMouseEnter={e=>{e.currentTarget.style.borderColor=`${sc}55`;e.currentTarget.style.boxShadow=`0 6px 20px ${sc}15`}}
                        onMouseLeave={e=>{e.currentTarget.style.borderColor=`${sc}22`;e.currentTarget.style.boxShadow='none'}}>
                        <div style={{position:'absolute',left:0,top:0,bottom:0,width:3,background:`linear-gradient(180deg,${sc},transparent)`}}/>
                        <div style={{width:42,height:42,borderRadius:11,background:`linear-gradient(135deg,${sc},${sc}aa)`,color:'#000',display:'flex',alignItems:'center',justifyContent:'center',fontSize:18,flexShrink:0,fontFamily:"'Bebas Neue',sans-serif",boxShadow:`0 4px 12px ${sc}40`}}>
                          {(m.name||'?')[0].toUpperCase()}
                        </div>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:3,flexWrap:'wrap'}}>
                            <span style={{fontSize:13,fontWeight:700,color:'#f0ece8'}}>{m.name||'(no name)'}</span>
                            <span style={{fontSize:8,fontWeight:800,padding:'2px 8px',borderRadius:50,background:`${sc}22`,color:sc,border:`1px solid ${sc}44`,letterSpacing:'0.08em',textTransform:'uppercase'}}>
                              {getStatusIcon(state)} {getStatusLabel(state)}
                            </span>
                            {expiringSoon && !isExpired && !isPaused && (
                              <span style={{fontSize:8,fontWeight:800,padding:'2px 8px',borderRadius:50,background:'rgba(251,146,60,0.15)',color:'#fb923c',border:'1px solid rgba(251,146,60,0.3)',letterSpacing:'0.08em'}}>⚠ EXPIRING SOON</span>
                            )}
                          </div>
                          <div style={{fontSize:11,color:'#888',display:'flex',gap:14,flexWrap:'wrap'}}>
                            <span>📧 {m.email||'—'}</span>
                            {m.membership?.expiresAt && <span>📅 Expires <strong style={{color:'#bbb'}}>{fmtExpiry(m.membership)}</strong></span>}
                            {days !== null && !isPaused && (
                              <span style={{color:isExpired?'#e84a2f':days<=7?'#fb923c':'#888'}}>⏱ {fmtRemaining(m.membership)}</span>
                            )}
                            {isPaused && m.membership?.pausedAt && (
                              <span style={{color:'#42a5f5'}}>⏸ Paused</span>
                            )}
                          </div>
                        </div>
                        {/* Reminder action — coach can nudge expiring/expired members */}
                        {(isExpired || expiringSoon) && (
                          <button onClick={()=>sendMembershipReminder(m)} disabled={alreadyReminded}
                            title={alreadyReminded?'Already reminded this session':'Send a renewal nudge'}
                            style={{background:alreadyReminded?'rgba(255,255,255,0.04)':isExpired?'linear-gradient(135deg,#e84a2f,#c93820)':'linear-gradient(135deg,#fb923c,#ea7a1a)',color:alreadyReminded?'#555':'#fff',border:'none',borderRadius:50,padding:'9px 16px',fontSize:10,fontWeight:800,letterSpacing:'0.06em',cursor:alreadyReminded?'not-allowed':'pointer',flexShrink:0,boxShadow:alreadyReminded?'none':isExpired?'0 4px 12px rgba(232,74,47,0.35)':'0 4px 12px rgba(251,146,60,0.35)',transition:'all 0.2s'}}>
                            {alreadyReminded?'✓ SENT':'📣 REMIND'}
                          </button>
                        )}
                      </div>
                    )
        ...
        {/* FORUM — embedded community forum with coach moderation powers */}
        {tab==='forum' && (
          <Forum embedded currentRole="coach" />
        )}
          ...
          <div style={{...glass(),width:'100%',maxWidth:480,maxHeight:'85vh',display:'flex',flexDirection:'column',border:'1px solid rgba(66,165,245,0.25)'}}>
            <div style={{padding:'14px 18px',borderBottom:'1px solid rgba(255,255,255,0.05)',display:'flex',alignItems:'center',gap:10}}>
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:9,color:'#555',textTransform:'capitalize'}}>{msgTarget.role||'Member'} · {getMemberLevel(msgTarget)}</div>
              </div>
