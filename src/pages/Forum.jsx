...
const CAT_COLORS = { General: '#f5c842', Training: '#e84a2f', Nutrition: '#22c55e', Equipment: '#42a5f5', Events: '#c084fc', Other: '#888' }
...
export default function Forum({ embedded = false, currentRole = 'member' }) {
  ...
  const inp = { background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, padding: '12px 16px', color: '#f0ece8', fontSize: 13, fontFamily: 'Montserrat,sans-serif', outline: 'none', width: '100%', boxSizing: 'border-box', transition: 'all 0.3s' }
  ...
  const forumContent = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {/* Hero Header */}
      <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 24, background: 'linear-gradient(135deg,#1a1413 0%,#0e0a0a 60%,rgba(232,74,47,0.08) 100%)', border: '1px solid rgba(232,74,47,0.12)', padding: '28px 30px', boxShadow: '0 16px 60px rgba(0,0,0,0.5)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 14, position: 'relative', zIndex: 1 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 4 }}>
              ...
              <div>
                <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 28, letterSpacing: '0.06em', color: '#f0ece8', lineHeight: 1 }}>COMMUNITY FORUM</div>
                <div style={{ fontSize: 10, color: '#666', letterSpacing: '0.08em', fontWeight: 600, marginTop: 2 }}>ASK QUESTIONS • SHARE KNOWLEDGE • CONNECT</div>
              </div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ display: 'flex', gap: 8 }}>
              <div style={{ textAlign: 'center', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: '8px 16px' }}>
                <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 20, color: '#f5c842' }}>{posts.length}</div>
                <div style={{ fontSize: 8, color: '#666', fontWeight: 700, letterSpacing: '0.1em' }}>POSTS</div>
              </div>
              <div style={{ textAlign: 'center', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: '8px 16px' }}>
                <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 20, color: '#42a5f5' }}>{posts.reduce((a, p) => a + (p.replies || []).length, 0)}</div>
                <div style={{ fontSize: 8, color: '#666', fontWeight: 700, letterSpacing: '0.1em' }}>REPLIES</div>
              </div>
            </div>
            <button onClick={() => { setShowNewPost(v => !v); setActivePost(null) }}
              style={{ background: showNewPost ? 'rgba(255,255,255,0.06)' : 'linear-gradient(135deg,#e84a2f,#c93820)', color: showNewPost ? '#888' : '#fff', border: showNewPost ? '1px solid rgba(255,255,255,0.1)' : 'none', borderRadius: 14, padding: '14px 28px', fontSize: 12, fontWeight: 800, letterSpacing: '0.06em', cursor: 'pointer', boxShadow: showNewPost ? 'none' : '0 8px 28px rgba(232,74,47,0.5)', transition: 'all 0.3s cubic-bezier(0.34,1.56,0.64,1)' }}
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ position: 'relative', flex: 1, minWidth: 200 }}>
          <span style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', fontSize: 13, color: '#555', pointerEvents: 'none' }}>🔍</span>
          <input placeholder="Search posts, topics, or authors…" value={searchQ} onChange={e => setSearchQ(e.target.value)}
            style={{ ...inp, paddingLeft: 38, borderRadius: 14, background: 'rgba(20,15,14,0.7)', border: '1px solid rgba(255,255,255,0.08)' }}
            ...
            onBlur={e => { e.target.style.borderColor = 'rgba(255,255,255,0.08)'; e.target.style.boxShadow = 'none'; e.target.style.background = 'rgba(20,15,14,0.7)' }} />
        </div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {['All', ...CATEGORIES].map(c => {
            const active = filterCat === c
            const cc = c === 'All' ? '#f5c842' : (CAT_COLORS[c] || '#888')
            const icon = c === 'All' ? '🔥' : CAT_ICONS[c]
            return (
              <button key={c} onClick={() => setFilterCat(c)}
                style={{ display: 'flex', alignItems: 'center', gap: 5, background: active ? `${cc}18` : 'rgba(255,255,255,0.02)', color: active ? cc : '#555', border: active ? `1.5px solid ${cc}55` : '1px solid rgba(255,255,255,0.06)', borderRadius: 12, padding: '8px 14px', fontSize: 10, fontWeight: 700, cursor: 'pointer', transition: 'all 0.25s', boxShadow: active ? `0 4px 16px ${cc}22` : 'none', transform: active ? 'scale(1.02)' : 'scale(1)' }}
                ...
                onMouseLeave={e => { if (!active) { e.currentTarget.style.borderColor = 'rgba(255,255,255,0.06)'; e.currentTarget.style.color = '#555'; e.currentTarget.style.background = 'rgba(255,255,255,0.02)' } }}>
      ...
      {showNewPost && (
        <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 22, background: 'linear-gradient(135deg,rgba(22,20,20,0.98),rgba(14,12,12,0.99))', border: '1px solid rgba(232,74,47,0.25)', padding: '28px 30px', boxShadow: '0 16px 60px rgba(0,0,0,0.5), 0 0 40px rgba(232,74,47,0.08)' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
            ...
            <div>
              <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 20, letterSpacing: '0.06em', color: '#f0ece8' }}>CREATE A POST</div>
              <div style={{ fontSize: 10, color: '#666', letterSpacing: '0.06em', fontWeight: 600 }}>Ask a question or start a discussion</div>
            </div>
          </div>

          <div style={{ display: 'flex', gap: 14, marginBottom: 16 }}>
            <div style={{ flex: 2 }}>
              <label style={{ fontSize: 9, color: '#888', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Title *</label>
            ...
            <div style={{ flex: 1 }}>
              <label style={{ fontSize: 9, color: '#888', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Category</label>
              <select value={newCategory} onChange={e => setNewCategory(e.target.value)}
                ...
                {CATEGORIES.map(c => <option key={c} value={c} style={{ background: '#1a1818', color: '#f0ece8' }}>{CAT_ICONS[c]} {c}</option>)}
              </select>
            </div>
          </div>
          <div style={{ marginBottom: 20 }}>
            <label style={{ fontSize: 9, color: '#888', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', display: 'block', marginBottom: 6 }}>Body *</label>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <span style={{ fontSize: 10, color: '#555' }}>Visible to all members and coaches</span>
          </div>
        </div>
      )}
      ...
      {activePost ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <button onClick={() => setActivePost(null)}
            style={{ alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: 6, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '10px 20px', fontSize: 11, fontWeight: 700, color: '#888', cursor: 'pointer', transition: 'all 0.2s' }}
            onMouseEnter={e => { e.currentTarget.style.color = '#f0ece8'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.2)'; e.currentTarget.style.background = 'rgba(255,255,255,0.05)' }}
            onMouseLeave={e => { e.currentTarget.style.color = '#888'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)'; e.currentTarget.style.background = 'rgba(255,255,255,0.03)' }}>
            <span style={{ fontSize: 14 }}>←</span> BACK TO FORUM
          </button>

          {/* Original post */}
          <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 22, background: 'linear-gradient(135deg,#1a1413 0%,#0e0a0a 100%)', border: '1px solid rgba(245,200,66,0.15)', padding: '26px 28px', boxShadow: '0 16px 60px rgba(0,0,0,0.5)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 14, marginBottom: 16 }}>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 9, fontWeight: 800, padding: '4px 12px', borderRadius: 8, background: `${CAT_COLORS[activePost.category] || '#888'}15`, color: CAT_COLORS[activePost.category] || '#888', border: `1px solid ${CAT_COLORS[activePost.category] || '#888'}33`, letterSpacing: '0.08em', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 4 }}>
                ...
                <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 28, letterSpacing: '0.04em', color: '#f0ece8', lineHeight: 1.2, marginBottom: 10 }}>{activePost.title}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 11, color: '#888' }}>
                  ...
                  <span style={{ fontWeight: 700, color: '#bbb' }}>{activePost.authorName}</span>
            ...
            <div style={{ fontSize: 13, color: '#ccc', lineHeight: 1.9, whiteSpace: 'pre-wrap', padding: '16px 18px', background: 'rgba(255,255,255,0.02)', borderRadius: 14, border: '1px solid rgba(255,255,255,0.04)' }}>{activePost.body}</div>

            <div style={{ marginTop: 16, display: 'flex', alignItems: 'center', gap: 12 }}>
              <button onClick={(e) => toggleLike(activePost.id, e)}
                style={{ display: 'flex', alignItems: 'center', gap: 6, background: (activePost.likes || []).includes(me?.uid) ? 'rgba(232,74,47,0.12)' : 'rgba(255,255,255,0.03)', border: `1px solid ${(activePost.likes || []).includes(me?.uid) ? 'rgba(232,74,47,0.3)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 12, padding: '8px 16px', fontSize: 12, fontWeight: 700, color: (activePost.likes || []).includes(me?.uid) ? '#e84a2f' : '#888', cursor: 'pointer', transition: 'all 0.2s' }}
                onMouseEnter={e => { e.currentTarget.style.transform = 'scale(1.05)' }}
                onMouseLeave={e => { e.currentTarget.style.transform = 'scale(1)' }}>
                {(activePost.likes || []).includes(me?.uid) ? '❤️' : '🤍'} <span>{(activePost.likes || []).length}</span>
              </button>
              <span style={{ fontSize: 11, color: '#666', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5 }}>💬 {(activePost.replies || []).length} repl{(activePost.replies || []).length === 1 ? 'y' : 'ies'}</span>
            </div>
          </div>
          ...
          {(activePost.replies || []).length > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              {(activePost.replies || []).map((r, i) => {
                ...
                return (
                  <div key={r.id || i}
                    style={{ position: 'relative', overflow: 'hidden', borderRadius: 18, background: isCoachReply ? `linear-gradient(135deg,rgba(22,20,20,0.98),${accentColor}06)` : 'linear-gradient(135deg,rgba(22,20,20,0.97),rgba(14,12,12,0.99))', border: `1px solid ${isCoachReply ? `${accentColor}25` : 'rgba(255,255,255,0.05)'}`, padding: '16px 20px', boxShadow: isCoachReply ? `0 8px 24px ${accentColor}08` : '0 4px 20px rgba(0,0,0,0.3)' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 12, fontWeight: 700, color: isCoachReply ? accentColor : '#bbb' }}>{r.authorName}</span>
                        ...
                        <span style={{ fontSize: 10, color: '#555' }}>{timeAgo({ seconds: new Date(r.createdAt).getTime() / 1000 })}</span>
                      </div>
                      {canDelete(r) && (
          ...
          <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 20, background: 'linear-gradient(135deg,rgba(22,20,20,0.98),rgba(66,165,245,0.03))', border: '1px solid rgba(66,165,245,0.15)', padding: '20px 22px', boxShadow: '0 8px 32px rgba(0,0,0,0.3)' }}>
            ...
            <button onClick={postReply} disabled={replying || !replyText.trim()}
              style={{ background: replyText.trim() ? 'linear-gradient(135deg,#42a5f5,#1e88e5)' : 'rgba(255,255,255,0.04)', color: replyText.trim() ? '#fff' : '#555', border: 'none', borderRadius: 12, padding: '12px 28px', fontSize: 11, fontWeight: 800, letterSpacing: '0.06em', cursor: replyText.trim() ? 'pointer' : 'not-allowed', boxShadow: replyText.trim() ? '0 6px 20px rgba(66,165,245,0.4)' : 'none', transition: 'all 0.3s', opacity: replying ? 0.7 : 1 }}
        ...
        <>
          {filtered.length === 0 ? (
            <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 24, background: 'linear-gradient(135deg,#1a1413,#0e0a0a)', border: '1px dashed rgba(255,255,255,0.06)', padding: '70px 30px', textAlign: 'center', boxShadow: '0 16px 60px rgba(0,0,0,0.4)' }}>
              ...
              <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 28, color: '#f0ece8', letterSpacing: '0.06em', marginBottom: 8 }}>{searchQ || filterCat !== 'All' ? 'NO MATCHING POSTS' : 'NO POSTS YET'}</div>
              <div style={{ fontSize: 12, color: '#666', letterSpacing: '0.04em', maxWidth: 300, margin: '0 auto' }}>{searchQ || filterCat !== 'All' ? 'Try a different search or category filter' : 'Be the first to start a discussion!'}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {filtered.map(p => {
                const cc = CAT_COLORS[p.category] || '#888'
                const liked = (p.likes || []).includes(me?.uid)
                ...
                return (
                  <div key={p.id} onClick={() => { setActivePost(p); setShowNewPost(false) }}
                    style={{ position: 'relative', overflow: 'hidden', borderRadius: 18, background: 'linear-gradient(135deg,rgba(22,20,20,0.97),rgba(14,12,12,0.99))', border: '1px solid rgba(255,255,255,0.06)', padding: '18px 22px', cursor: 'pointer', transition: 'all 0.3s cubic-bezier(0.34,1.56,0.64,1)', boxShadow: '0 4px 20px rgba(0,0,0,0.3)' }}
                    ...
                    <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 15, fontWeight: 700, color: '#f0ece8', lineHeight: 1.3, marginBottom: 5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.title}</div>
                        <div style={{ fontSize: 11, color: '#777', lineHeight: 1.5, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{p.body}</div>
                        <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 10, fontSize: 10, color: '#555' }}>
                          <span style={{ fontWeight: 700, color: '#888' }}>{p.authorName}</span>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, flexShrink: 0, paddingTop: 6 }}>
                        <button onClick={(e) => toggleLike(p.id, e)}
                          style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, background: liked ? 'rgba(232,74,47,0.1)' : 'rgba(255,255,255,0.03)', border: `1px solid ${liked ? 'rgba(232,74,47,0.25)' : 'rgba(255,255,255,0.06)'}`, borderRadius: 10, padding: '8px 12px', cursor: 'pointer', transition: 'all 0.2s' }}>
                          <span style={{ fontSize: 14 }}>{liked ? '❤️' : '🤍'}</span>
                          <span style={{ fontSize: 10, fontWeight: 700, color: liked ? '#e84a2f' : '#666' }}>{(p.likes || []).length}</span>
                        </button>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 10, padding: '8px 12px' }}>
                          <span style={{ fontSize: 14 }}>💬</span>
                          <span style={{ fontSize: 10, fontWeight: 700, color: replyCount > 0 ? '#42a5f5' : '#555' }}>{replyCount}</span>
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#1a1a1a', fontFamily: 'Montserrat,sans-serif', position: 'relative' }}>
