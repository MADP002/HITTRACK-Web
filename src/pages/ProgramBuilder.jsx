...
export default function ProgramBuilder() {
  ...
  const [form, setForm] = useState({
    ...
    goal: '', daysPerWeek: 3,
    injuries: '',
    medicalCert: null,  // populated by MedicalCertUpload writes; persists in users/{uid} directly
  })
  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        // Re-do users may already have a cert from a prior session — keep it
        // visible in the wizard so they can replace or leave as-is.
        if (p.medicalCert) patches.medicalCert = p.medicalCert
  ...
  function handleNext() {
    ...
    // Create initial stats doc so member appears on leaderboard immediately.
    // IMPORTANT: do NOT seed totalWorkouts/streak/weeklyPct here. training-complete.jsx
    // (mobile) is the single source of truth for those fields. If this screen runs
    // again on Re-do, we must not wipe real progress back to zero. Reads already
    // fall back to 0 safely when these fields are absent.
    const user = auth.currentUser
    if (user) {
      ...
      // Generate and persist the real training program to workouts/{uid}.
      // Mobile's training-complete.jsx reads workouts/{uid}.trainingProgram —
      // without this write, a web-signup member cannot train on mobile.
      try {
        const tp = generateTrainingProgram(profile.stance, profile.goal, profile.experience)
        setDoc(doc(db, 'workouts', user.uid), {
          trainingProgram:         tp.trainings,
          trainingCurrentLevel:    tp.currentLevel,
          trainingLevelStars:      tp.levelStars,
          trainingCurrentIndex:    0,
          trainingCompletedLevels: [],
          trainingGoal:            tp.goal,
          trainingStance:          tp.stance,
          trainingGeneratedAt:     tp.generatedAt,
        }, { merge: true }).catch(e => console.warn('Training program save skipped:', e.message))
      } catch (e) {
        console.warn('Training program generation failed:', e.message)
      }
    }
...
function StepBody({ form, update, errors, bmi, bmiLabel, bmiColor }) {
  return (
    <div style={s.stepContent}>
      ...
      {/* Medical certificate — optional. Writes directly to users/{uid}.medicalCert
          so the cert persists even if the member abandons the wizard. */}
      <MedicalCertUpload
        member={{ medicalCert: form.medicalCert }}
        onUploaded={(cert) => update('medicalCert', cert)}
      />
    </div>
  )
}
...
function StepGoals({ form, update, errors }) {
  return (
    <div style={s.stepContent}>
      ...
      {/* Preview program — first 5 real Training Lab movements, stance-resolved */}
      {form.experience && form.goal && PROGRAM_TEMPLATES[form.goal] && (() => {
        const stanceMap = {
          Orthodox: { lead: 'Left',  rear: 'Right' },
          Southpaw: { lead: 'Right', rear: 'Left'  },
        }
        const sm = stanceMap[form.stance] || stanceMap['Orthodox']
        const seq = PROGRAM_TEMPLATES[form.goal].sequence
        ...
        return (
          <div style={{ background: 'rgba(245,200,66,0.04)', border: '1px solid rgba(245,200,66,0.12)', borderRadius: 14, padding: '16px 18px' }}>
            ...
            {first5.map((movId, i) => {
              ...
              if (mov.hand === 'lead') {
                displayName = `${sm.lead} ${movId.replace('lead_', '').split('_').map(w => w[0].toUpperCase() + w.slice(1)).join(' ')}`
              } else if (mov.hand === 'rear') {
                displayName = `${sm.rear} ${movId.replace('rear_', '').split('_').map(w => w[0].toUpperCase() + w.slice(1)).join(' ')}`
              }
...
function DoneScreen({ navigate, form, bmi, bmiLabel, bmiColor }) {
  const isMobile = useIsMobile()
  // Full training program — same sequence mobile will unlock progressively in Training Lab.
  // Stance-resolved hand labels so Orthodox vs Southpaw actually differ.
  const stanceMap = {
    Orthodox: { lead: 'Left',  rear: 'Right' },
    Southpaw: { lead: 'Right', rear: 'Left'  },
  }
  const sm = stanceMap[form.stance] || stanceMap['Orthodox']
  const sequence = PROGRAM_TEMPLATES[form.goal]?.sequence || PROGRAM_TEMPLATES['Learn Boxing'].sequence
  const program = sequence.map(movId => {
    const mov = MOVEMENT_LIBRARY[movId]
    if (!mov) return null
    let displayName = movId.split('_').map(w => w[0].toUpperCase() + w.slice(1)).join(' ')
    if (mov.hand === 'lead') {
