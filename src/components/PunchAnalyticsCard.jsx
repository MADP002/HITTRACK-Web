    ...
    <div style={glass()}>
      <div style={{padding:'18px 22px 14px',borderBottom:'1px solid rgba(245,200,66,0.08)',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
        <div>
          ...
          <div style={{fontSize:11,color:'var(--t-dim3)',marginTop:2}}>Powered by AI Pose Detection</div>
        </div>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          {isPose && (
            <span style={{fontSize:8,fontWeight:700,color:'var(--a-green2)',background:'rgba(74,222,128,0.1)',border:'1px solid rgba(74,222,128,0.25)',borderRadius:50,padding:'3px 8px',letterSpacing:'0.08em'}}>LIVE</span>
          )}
          {isRec && (
            <span title="Derived from coach-submitted training reports. Live per-frame metrics arrive with the full pose pipeline."
              style={{fontSize:8,fontWeight:700,color:'var(--a-gold)',background:'rgba(245,200,66,0.1)',border:'1px solid rgba(245,200,66,0.25)',borderRadius:50,padding:'3px 8px',letterSpacing:'0.08em'}}>SESSION SUMMARY</span>
          )}
          <div style={{fontSize:9,fontWeight:700,color:'var(--a-purple)',background:'rgba(192,132,252,0.1)',border:'1px solid rgba(192,132,252,0.25)',borderRadius:50,padding:'3px 10px',letterSpacing:'0.08em'}}>TENSORFLOW</div>
        </div>
      </div>
      <div style={{padding:'20px',display:'flex',flexDirection:'column',gap:14}}>

        {loading ? (
          <div style={{textAlign:'center',padding:'20px',color:'var(--t-dim3)',fontSize:11}}>Loading pose data...</div>
        ) : source === 'none' ? (
          <>
            <div style={{background:'rgba(192,132,252,0.06)',border:'1px dashed rgba(192,132,252,0.25)',borderRadius:16,padding:'20px',textAlign:'center',display:'flex',flexDirection:'column',gap:8}}>
              ...
              <div style={{fontSize:13,fontWeight:700,color:'var(--a-purple)'}}>Live Punch Detection</div>
              <div style={{fontSize:11,color:'var(--t-muted)',lineHeight:1.7}}>
                Punch data is captured in real-time through the <strong style={{color:'var(--a-purple)'}}>mobile app</strong> using TensorFlow pose detection. Complete a live training session to populate analytics.
              </div>
              <div style={{background:'rgba(192,132,252,0.08)',border:'1px solid rgba(192,132,252,0.15)',borderRadius:10,padding:'10px 14px',marginTop:4,fontSize:11,color:'var(--t-muted)',display:'flex',alignItems:'center',gap:8,justifyContent:'center'}}>
                <span>🔴</span><span>No training sessions yet — awaiting first session</span>
              </div>
            </div>
            {METRIC_DEFS.map((p,i)=>(
              <div key={p.key} style={{display:'flex',alignItems:'center',gap:12,padding:'10px 12px',background:'var(--t-s02)',borderRadius:12,border:'1px solid var(--t-s04)'}}>
                <span style={{fontSize:20,opacity:0.4}}>{p.icon}</span>
                <div style={{flex:1}}>
                  <div style={{display:'flex',justifyContent:'space-between',marginBottom:5}}>
                    <span style={{fontSize:11,fontWeight:700,color:'var(--t-dim3)'}}>{p.label}</span>
                    <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:14,color:'var(--t-dim4)'}}>—</span>
                  </div>
                  <div style={{height:5,background:'var(--t-s04)',borderRadius:50,overflow:'hidden'}}>
          ...
          <>
            <div style={{background:'rgba(74,222,128,0.05)',border:'1px solid rgba(74,222,128,0.15)',borderRadius:14,padding:'12px 16px',display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                ...
                <span style={{fontSize:11,fontWeight:700,color:'var(--a-green2)'}}>
                  {data.totalSessions} {isPose ? 'pose session' : 'session'}{data.totalSessions!==1?'s':''} recorded
                </span>
              </div>
              {data.lastSessionAt && (
                <span style={{fontSize:10,color:'var(--t-dim3)'}}>Last: {data.lastSessionAt.toLocaleDateString('en-US',{month:'short',day:'numeric'})}</span>
              )}
            </div>
                ...
                <div key={p.key} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 14px',background:`${p.color}08`,borderRadius:12,border:`1px solid ${p.color}20`,transition:'all 0.3s',opacity:mounted?1:0,transform:mounted?'translateX(0)':'translateX(-8px)'}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{display:'flex',justifyContent:'space-between',marginBottom:6}}>
                      <span style={{fontSize:11,fontWeight:700,color: isMissing ? 'var(--t-dim3)' : 'var(--t-dim1)'}}>{p.label}</span>
                      {isMissing ? (
                        <span title="Not enough reps logged yet"
                          style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:'var(--t-dim4)',cursor:'help'}}>—</span>
                      ) : (
                        <span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:18,color:p.color}}>
                          <AnimNum target={parseFloat(raw)}/>
                          <span style={{fontSize:10,color:'var(--t-dim3)',fontFamily:'Montserrat,sans-serif',marginLeft:2}}>{unit}</span>
                        </span>
                      )}
                    </div>
                    <div style={{height:6,background:'var(--t-s06)',borderRadius:50,overflow:'hidden'}}>
                      {isMissing
                        ? <div style={{height:'100%',width:'100%',background:'var(--t-s04)',borderRadius:50}}/>
            ...
            {isPose && data.formBreakdown && (
              <div style={{marginTop:4}}>
                <div style={{fontSize:10,fontWeight:700,color:'var(--t-dim2)',letterSpacing:'0.1em',textTransform:'uppercase',marginBottom:10}}>Form Breakdown by Punch</div>
                <div style={{display:'flex',flexDirection:'column',gap:8}}>
                  {Object.entries(data.formBreakdown).map(([punch, score],i)=>{
                    const punchColor = {jab:'#f5c842',cross:'#e84a2f',hook:'#c084fc',uppercut:'#42a5f5'}[punch]||'var(--t-dim2)'
                    return(
            ...
            {!compact && data.recentSessions.length > 0 && (
              <div style={{marginTop:6}}>
                <div style={{fontSize:10,fontWeight:700,color:'var(--t-dim2)',letterSpacing:'0.1em',textTransform:'uppercase',marginBottom:10}}>Recent Sessions</div>
                <div style={{display:'flex',flexDirection:'column',gap:6}}>
                  {data.recentSessions.map((s)=>(
                    <div key={s.id} style={{display:'flex',alignItems:'center',gap:10,padding:'9px 12px',background:'var(--t-s02)',borderRadius:10,border:'1px solid var(--t-s04)'}}>
                      ...
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontSize:11,fontWeight:700,color:'var(--t-dim1)'}}>
                          {s.date ? s.date.toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric'}) : '—'}
                        </div>
                        <div style={{fontSize:9,color:'var(--t-dim3)',marginTop:1}}>{Math.round((s.duration||0)/60)}min · {s.totalPunches||0} {isPose ? 'punches' : 'reps'}</div>
                      </div>
                      <div style={{textAlign:'right'}}>
                        ...
                        <div style={{fontSize:8,color:'var(--t-dim3)',fontWeight:600}}>ACCURACY</div>
                      </div>
                    </div>
                  ))}
                </div>
