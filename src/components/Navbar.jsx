...
import { getTheme, toggleTheme, clearAppStorageKeepTheme } from '../lib/theme'
...
export default function Navbar({ user }) {
  ...
  // Light/dark toggle — flips a data-theme attribute on <html>, so the
  // whole member area re-themes instantly (no reload).
  const [theme, setThemeState] = useState(getTheme())
  function flipTheme() { setThemeState(toggleTheme()) }
      ...
      <nav className="nav" style={isMobile ? { padding:'10px 14px', justifyContent:'space-between' } : undefined}>
        ...
        {!isMobile && (
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            ...
            <button onClick={flipTheme} title={theme==='light'?'Switch to dark mode':'Switch to light mode'} aria-label="Toggle theme"
              style={{width:36,height:36,borderRadius:'50%',background:'var(--t-s04)',border:'1px solid var(--t-s08)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:15,cursor:'pointer',transition:'all 0.2s'}}
              onMouseEnter={e=>e.currentTarget.style.background='var(--t-s08)'}
        ...
        {isMobile && (
          <div style={{display:'flex',alignItems:'center',gap:6}}>
            ...
            <button onClick={flipTheme} title="Toggle theme" aria-label="Toggle theme"
              style={{width:38,height:38,borderRadius:'50%',background:'var(--t-s04)',border:'1px solid var(--t-s08)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:16,cursor:'pointer'}}>
              {theme==='light'?'🌙':'☀️'}
            </button>
            <button onClick={()=>setMenuOpen(o=>!o)} aria-label="Menu"
              style={{
                width:42,height:42,borderRadius:11,
                background:menuOpen?'rgba(232,74,47,0.18)':'var(--t-s04)',
                border:`1px solid ${menuOpen?'rgba(232,74,47,0.35)':'var(--t-s08)'}`,
              ...
              {[0,1,2].map(i=>(
                <span key={i} style={{
                  ...
                  background:menuOpen?'#e84a2f':'var(--t-text)',
      ...
      {isMobile && menuOpen && (
        <>
          ...
          <div style={{
            ...
            <div style={{padding:'22px 18px',borderBottom:'1px solid var(--t-s05)',display:'flex',alignItems:'center',gap:12,background:'linear-gradient(135deg,rgba(232,74,47,0.08),transparent 70%)'}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:13,fontWeight:700,color:'var(--t-text)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{user?.name||'Member'}</div>
                <div style={{fontSize:10,color:'var(--t-dim3)',marginTop:2,letterSpacing:'0.04em',textTransform:'uppercase',fontWeight:600}}>Tap to view profile</div>
              </div>
            </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',backdropFilter:'blur(8px)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'Montserrat,sans-serif',padding:16}}>
          <div style={{background:'linear-gradient(135deg,var(--t-card),var(--t-card2))',borderRadius:20,border:'1px solid rgba(232,74,47,0.2)',boxShadow:'0 20px 60px rgba(0,0,0,0.6)',padding:isMobile?'30px 24px':'40px',maxWidth:380,width:'100%',textAlign:'center'}}>
            ...
            <div style={{display:'flex',gap:12,justifyContent:'center'}}>
              <button onClick={()=>setShowLogout(false)} style={{background:'transparent',color:'var(--t-dim3)',border:'1.5px solid var(--t-s10)',borderRadius:50,padding:'10px 24px',fontSize:13,fontWeight:700,cursor:'pointer'}}>Cancel</button>
